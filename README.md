...
OmniSource 面向福建九地市非遗文化展示、问答、共创和沉浸式浏览场景，将非遗项目组织为可对话、可协作、可生成图像、可进入 WebGL/3D 展示空间的文化 Agent。项目当前包含 Spring Boot 后端、Vue 3 展示端、UniApp 移动端、RAG 数据工具，以及比赛说明书和截图提示词材料。

## 核心能力

- **福建九地市非遗入口**：Vue 首页通过福建地图和 27 条本地画廊数据，把九地市非遗项目按城市分组展示。
- **多智能体非遗群聊**：用户可创建聊天室，选择最多 6 个文化 Agent；后端根据知识范围、上下文和冷却时间选择 1-3 个 Agent 流式回复。
...
- **沉浸式前端**：Vue 3 前端包含首页地图、Agent 聊天室、非遗关系图谱、OGL 无限画廊和 Three.js 模型展示。
...
- `/agent`：聊天室、Agent 选择、WebSocket 多智能体流式对话、绑定 Agent 生图。
...
2. 展示底部非遗画廊和右侧翻转卡片说明。
3. 进入 Agent 页面，创建包含 2-3 个文化 Agent 的聊天室。
4. 输入 `你们一起介绍一下寿山石雕的历史和工艺特点`，展示多 Agent 流式回复。
5. 需要外部资料时再打开联网搜索。
6. 选择聊天室内 Agent，输入视觉描述并生成图片。
7. 展示图片进入聊天流、OSS 归档和历史消息。
8. 打开 `/graph`、`/infinite-grid` 或 `/data` 展示关系图谱与沉浸式视觉页面。
9. 使用 `/api/system-profile` 辅助解释系统架构。

## 材料与截图

- `截图/` 保存 Web、移动端截图、Logo、校徽和 AI 作图提示词。
...
- 不只是单轮聊天：项目将知识库、Agent 调度、RAG、AIGC、生图入聊、WebSocket 流式通信、OSS 归档和 WebGL 展示串成闭环。
