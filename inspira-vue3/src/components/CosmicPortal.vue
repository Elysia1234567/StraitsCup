...
const props = withDefaults(
  defineProps<PortalParams & { class?: string; containerClass?: string }>(),
  {
    portalComplexity: 4,
    crystalCount: 12,
    primaryColor: '#9b59b6',
...
let materials: Array<THREE.Material | THREE.ShaderMaterial> = [];
let portalMaterials: Array<THREE.ShaderMaterial | { uniforms?: Record<string, { value: unknown }> }> = [];
...
function addPortalShader(material: THREE.MeshPhysicalMaterial) {
...
function createVortexRings() {
  const colors = [params.value.primaryColor, params.value.secondaryColor, params.value.accentColor, params.value.vortexColor];
...
function createFloatingCrystals() {
  const colors = [params.value.accentColor, params.value.vortexColor, params.value.primaryColor, params.value.secondaryColor];
...
function handleResize() {
  if (!containerRef.value || !camera || !renderer || !composer || !fxaaPass) return;
  const width = containerRef.value.clientWidth;
  const height = containerRef.value.clientHeight;
...
function animate() {
  ...
  portalMaterials.forEach((matLike) => {
    const uniforms = matLike.uniforms;
    if (!uniforms) return;
    if (uniforms.time) uniforms.time.value = time;
    if (uniforms.dimensionShift) uniforms.dimensionShift.value = params.value.dimensionShift;
  });
...
function activatePortal() {
  // Make interaction immediately visible: strong pulse + short bloom boost.
  portalMaterials.forEach((matLike) => {
    if (matLike.uniforms?.pulseTime) matLike.uniforms.pulseTime.value = time;
  });
  params.value.dimensionShift = Math.random();
  if (bloomPass) {
    const original = bloomPass.strength;
    bloomPass.strength = Math.min(original + 0.9, 3);
    setTimeout(() => {
...
function shiftDimensions() {
  ...
  params.value.dimensionShift = Math.random();
  params.value.rotationSpeed = 0.2 + Math.random() * 1.6;
  createPortalScene();
  activatePortal();
}
...
<template>
  <div
    ...
    :class="[props.class, props.containerClass]"
  >
    <canvas
      ref="canvasRef"
