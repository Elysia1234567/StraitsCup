...
interface Props {
  ...
  class?: string;
  /** 图谱页等：去掉模糊光晕与扫描线叠层，便于透出清晰黑洞与线条 */
  sharp?: boolean;
  /**
   * 全屏底层背景：使用 fixed 铺满视口。
   * 默认 relative 会与外层传入的 fixed 冲突，导致背景参与文档流「竖着排列」。
   */
  cover?: boolean;
}
...
<template>
  ...
  >
    ...
    <canvas
      ...
      :class="sharp ? 'opacity-[0.22]' : 'opacity-20'"
    />
    <motion.div
      v-if="!sharp"
      class="pointer-events-none absolute top-1/2 left-1/2 z-3 h-[min(42vh,360px)] w-[min(10%,140px)] max-w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-full bg-size-[100%_220%] bg-position-[0%_100%] opacity-70 mix-blend-plus-lighter blur-2xl [background:linear-gradient(20deg,#00f8f1,#ffbd1e20_16.5%,#fe848f_33%,#fe848f20_49.5%,#00f8f1_66%,#00f8f160_85.5%,#ffbd1e_100%)_0_100%/100%_220%]"
      :animate="{ backgroundPosition: '0% 180%' }"
      :transition="{ duration: 18, ease: 'linear', repeat: Infinity }"
    />
    <div
      v-if="!sharp"
      class="absolute top-0 left-0 z-7 size-full opacity-50 mix-blend-overlay [background:repeating-linear-gradient(transparent,transparent_1px,rgba(255,255,255,0.15)_1px,rgba(255,255,255,0.15)_2px)]"
    />
  </div>
</template>
